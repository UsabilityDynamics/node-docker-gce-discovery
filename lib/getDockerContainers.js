  ...
  async.auto({
    ...
    get_obj_machine: ['get_machines', function(results, callback){
      ...
      async.each(results.get_machines, function(value, callback_each){
        ...
        machineBody.machine = value;
        var machineObj = {};
        machineObj[value.name] = _.extend( {
          tags: _.get( value, 'tags.items' )
        },  machineBody );
      ...
      async.each( results.get_obj_machine, function(value, callback_each1 ){
        ...
        _.each( value, function(value_inside, key_inside){
          ...
          instances.machines.push({
            ...
            privateAddress:  value_inside.modem.host,
            publicAddress:  _.get(value_inside, 'machine.networkInterfaces[0].accessConfigs[0].natIP'),                      
            dockerPort: value_inside.modem.port,
            tags: value_inside.tags
          });
    ...
    if(!error){
      ...
      done(null, result.get_containers,result);
    } else {
      done(error);
    }
