  ...
  async.auto({
    get_machines: [function(callback){
      ...
      gce.getInstances(options, function getMachines(error, machines){
        //console.log('GDC machines', require('util').inspect(machines, {showHidden: false, depth: 10, colors: true}));
    ...
    get_obj_machine: ['get_machines', function(results, callback){
      ...
      var ccMachines = false;
      _.each(_.get(options, 'machineTags'), function(value){
        if(value == 'client-cluster'){
          ccMachines = true;
        }
      });
    ...
    get_containers: ['get_obj_machine', function(results, callback){
      ...
      if(_.get(results, 'get_obj_machine[0].name')){
        var instances = {
          containers: [],
          machines: _.get(results, 'get_obj_machine'),
        };
